...
def _get_geoip_reader():
    ...
    try:
        ...
        # Kelias — iš nustatymų (config/settings.py: GEOIP_PATH), su
        # atsargine numatytąja vieta repozitorijoje.
        db_path = getattr(settings, 'GEOIP_PATH', '') or os.path.join(
            settings.BASE_DIR, 'data', 'GeoLite2-Country.mmdb')
        if os.path.exists(db_path):
            _geoip_reader = geoip2.database.Reader(db_path)
        else:
            print('[GeoIP] nerastas %s - salys bus Unknown' % db_path)
...
def tikras_ip(request):
    """Tikrasis lankytojo adresas, kai svetaine stovi uz nginx.

    REMOTE_ADDR tokiu atveju yra pats nginx (127.0.0.1), tad pirma ziurim
    i antrastes, kurias jis perduoda. X-Forwarded-For gali tureti grandine
    "klientas, proxy1, proxy2" - klientas yra pirmas.

    Antrastes rase nginx, ne narsykle (proxy_set_header X-Forwarded-For),
    tad jomis pasitikim; be to nustatymo visi lankytojai atrodytu kaip
    vienas 127.0.0.1.
    """
    xff = request.META.get('HTTP_X_FORWARDED_FOR', '')
    if xff:
        pirmas = xff.split(',')[0].strip()
        if pirmas:
            return pirmas
    tikras = (request.META.get('HTTP_X_REAL_IP', '') or '').strip()
    if tikras:
        return tikras
...
class VisitorTrackingMiddleware:
    ...
    def _maybe_log(self, request):
        ...
        try:
            from apps.analytics.models import VisitorHit, ip_maisa
            VisitorHit.objects.create(
                # Zalio IP NESAUGOM - tik vienakrypte maisa (privatumas).
                ip_hash=ip_maisa(ip),
