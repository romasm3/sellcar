...
def stripe_webhook(request):
    ...
    try:
        ...
        if event['type'] == 'checkout.session.completed':
            ...
            def _md(key, default=''):
                """Saugus metadata lauko skaitymas iš Stripe objekto."""
                try:
                    return md[key]
                except (KeyError, TypeError):
                    return getattr(md, key, default)

            if _md('type') == 'wallet_topup':
                user_id = int(_md('user_id', '0') or 0)
                amount = Decimal(str(_md('amount', '0') or '0'))
                bonus = Decimal(str(_md('bonus', '0') or '0'))

                # Idempotency — jau apdorota?
                ...
                # Pažymim pending topup tranzakciją kaip completed
                ...
                if tx:
                    ...
                    tx.stripe_payment_intent = payment_intent
                    tx.save(update_fields=[
                        'status', 'completed_at', 'stripe_payment_intent'
                    ])

                # Priskaičiuojam balansą
                user = User.objects.get(id=user_id)
                profile = user.profile
                current = profile.wallet_balance or Decimal('0')
                profile.wallet_balance = current + amount + bonus
                profile.save(update_fields=['wallet_balance'])

                # Bonus — atskira tranzakcija istorijai
                if bonus > 0:
                    WalletTransaction.objects.create(
                        user=user,
                        amount=bonus,
