...
def listing_pay_plan(request, pk, plan_code):
    ...
    # Highlight from plan (no separate addon yet — comes only via plan)
    ...
    if wallet_balance < total_price:
        messages.error(
            request,
            f'Insufficient wallet balance. Need ${total_price:.2f}, have ${wallet_balance:.2f}.'
        )
        return redirect('/accounts/wallet/')

    # ─── Deduct ───
    profile.wallet_balance = wallet_balance - total_price
    ...
    try:
        ...
        addon_parts = []
        if renew_cost > 0:
            addon_parts.append(f"{renew_count}* x {renew_days}d (${renew_cost:.2f})")
        if featured_cost > 0:
            addon_parts.append(f"featured {featured_days}d (${featured_cost:.2f})")

        desc = f"Listing #{listing.pk} - {plan['label']} plan"
        if addon_parts:
            desc += " + " + ", ".join(addon_parts)
    ...
    # ─── Activate listing ───
    listing.activate(days=plan['days'])

    now = timezone.now()
    if plan['boost_days'] > 0 or renew_count > 0:
    ...
    total_featured = plan['featured_days'] + featured_days

    # Stars (žvaigždutės ant kortelių) — atskira trukmė
    if final_star_count > 0:
        listing.star_level = 1
        listing.star_count = final_star_count
        if final_star_days > 0:
            # Pratęsiam ant viršaus jei jau yra aktyvus
    ...
    # Featured on homepage (Day's deals tab) — atskira trukmė
    if total_featured > 0:
        # Pratęsiam ant viršaus jei jau yra aktyvus
    ...
    _send_listing_published_email(listing, request.user)
    messages.success(request, f"Listing activated for {plan['days']} days!")

    return redirect(reverse('listing_success', kwargs={'pk': listing.pk}) + '?action=published')
